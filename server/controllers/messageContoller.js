...
export const getUsersForSidebar = async (req,res) => {
    try {
        const userId = req.user._id; //from authentication token middlware
        const filteredUsers = await User.find({_id: {$ne: userId }}).select("-password")
        
        // count number of messages not seen 
        const unseenMessages = {}
        const promises = filteredUsers.map(async (user)=> {
            const messages = await Message.find({//it will return an array 
              sendId:user._id, 
              receiverId:userId,
              seen: false
            })
            if(messages.length > 0){
                unseenMessages[user._id] = messages.length;//inshort creating a key and length of messages
            }
        })
...
export const getMessages = async(req,res)=>{
    try {
        ...
        const myId = req.user._id //user who has logged in
